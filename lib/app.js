        ...
        con.query(slotCheckSql, [roomId, today, timeSlot], (err, slotResults) => {
            ...
            con.query(bookingSql, [bookingId, studentId, roomId, today, timeSlot], (err, result) => {
                ...
                const updateSql = `
                    ...
                    SET status = 'pending', student_id = ?, booking_id = ?
                    WHERE room_id = ? AND availability_date = ? AND time_slot = ?
                `;
                
                con.query(updateSql, [studentId, bookingId, roomId, today, timeSlot], (err, updateResult) => {
                    if (err) {
...
// Update booking status (for lecturer approval)
app.put('/api/bookings/:bookingId/status', (req, res) => {
    ...
    con.query(sql, [status, approvedBy, bookingId], (err, result) => {
        ...
        con.query(updateAvailabilitySql, [availabilityStatus, bookingId], (err, updateResult) => {
            if (err) {
                console.error('Failed to update room availability:', err);
            }
